            ...
            for (const event of events) {
                ...
                if (result.success) {
                    await new ExchangeRecord({
                        txId: txID, fromAddress, toAddress: address,
                        usdtAmount: amount, rate: displayRate, trxAmount,
                        returnAddress, outTxHash: result.txID, status: 'completed'
                    }).save();
                    if (session && session.userId) {
                        await safew.sendMessage(session.userId, `✅ 请查收，${trxAmount} TRX 已转入您的地址`);
                    }
                    if (config.ADMIN_ID) {
